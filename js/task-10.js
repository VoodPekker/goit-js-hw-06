...
function createBoxes(amount) {
  ...
  for (let i = 0; i < amount; i += 1) {
    size += 10 * i;
    const div = `<div class="item" style="display: block; margin-right: 30px; margin-bottom: 30px; background-color: ${getRandomHexColor()}; width: ${size}px; height: ${size}px; border-radius: ${size}px;"></div>`;
    boxesArr.push(div);
  }
